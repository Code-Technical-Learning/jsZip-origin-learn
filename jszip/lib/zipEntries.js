...
let sig = require("./signature");
let ZipEntry = require("./zipEntry");
...
ZipEntries.prototype = {
    ...
    checkSignature: function (expectedSignature) {
        if (!this.reader.readAndCheckSignature(expectedSignature)) {
            this.reader.index -= 4;
            let signature = this.reader.readString(4);
            throw new Error(
    ...
    isSignature: function (askedIndex, expectedSignature) {
        let currentIndex = this.reader.index;
        this.reader.setIndex(askedIndex);
        let signature = this.reader.readString(4);
        let result = signature === expectedSignature;
    ...
    readBlockEndOfCentral: function () {
        ...
        let zipComment = this.reader.readData(this.zipCommentLength);
        let decodeParamType = support.uint8array ? "uint8array" : "array";
        // To get consistent behavior with the generation part, we will assume that
        // this is utf8 encoded unless specified otherwise.
        let decodeContent = utils.transformTo(decodeParamType, zipComment);
    ...
    readBlockZip64EndOfCentral: function () {
        ...
        let extraDataSize = this.zip64EndOfCentralSize - 44,
            index = 0,
            extraFieldId,
            extraFieldLength,
            extraFieldValue;
    ...
    readEndOfCentral: function () {
        ...
        if (offset < 0) {
            ...
            // get unreadable data.
            let isGarbage = !this.isSignature(0, sig.LOCAL_FILE_HEADER);
        ...
        let extraBytes = endOfCentralDirOffset - expectedEndOfCentralDirOffset;

        if (extraBytes > 0) {
            // console.warn(extraBytes, "extra bytes at beginning or within zipfile");
            if (
                this.isSignature(endOfCentralDirOffset, sig.CENTRAL_FILE_HEADER)
            ) {
