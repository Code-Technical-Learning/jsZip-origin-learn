...
let GenericWorker = require("./stream/GenericWorker");
let StreamHelper = require("./stream/StreamHelper");
let defaults = require("./defaults");
let CompressedObject = require("./compressedObject");
let ZipObject = require("./zipObject");
let generate = require("./generate");
let nodejsUtils = require("./nodejsUtils");
let NodejsStreamInputAdapter = require("./nodejs/NodejsStreamInputAdapter");
...
let fileAdd = function (name, data, originalOptions) {
    ...
    let o = utils.extend(originalOptions || {}, defaults);
 ...
 */
let parentFolder = function (path) {
    if (path.slice(-1) === "/") {
 ...
 */
let forceTrailingSlash = function (path) {
 ...
 */
let folderAdd = function (name, createFolders) {
    createFolders =
...
let out = {
    ...
    file: function (name, data, o) {
        if (arguments.length === 1) {
            if (isRegExp(name)) {
                ...
                // text
                let obj = this.files[this.root + name];
                if (obj && !obj.dir) {
                    return obj;
                } else {
                    return null;
    ...
    folder: function (arg) {
        ...
        let newFolder = folderAdd.call(this, name);
